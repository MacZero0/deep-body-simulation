 ...
 * @Date: 2020-12-21 18:21:13
 * @Last Modified by:   Mac
 * @Last Modified time: 2020-12-21 18:21:13
...
public class Matrix 
{
    ...
    public static Matrix Relu(Matrix IN, Matrix OUT){
        for (int row = 0; row < IN.GetRows(); row ++)
        {
            for (int col = 0; col < IN.GetCols(); col ++)
                OUT.SetValue(row, col, (IN.GetValue(row, col) <= 0)?0:IN.GetValue(row, col));
        }
        return OUT;
    }

    public static Matrix Layer(Matrix IN, Matrix W, Matrix b, Matrix OUT) {
    if(IN.GetCols() != W.GetRows() || W.GetCols() != b.GetCols() || IN.GetRows() != b.GetRows()) {
        Debug.Log("Incompatible dimensions for layer feed-forward.");
        return IN;
    } else {
        OUT = Product(IN, W, OUT);
        OUT = Add(OUT, b, OUT);
